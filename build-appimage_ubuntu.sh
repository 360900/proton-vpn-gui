#!/usr/bin/env bash
# build-appimage_ubuntu.sh
# Builds a self-contained AppImage on Ubuntu 24.04 (used by the CI release
# workflow).  Building natively on Ubuntu ensures that the bundled Python venv
# packages are compiled against the same ABI as the system Python, avoiding
# import failures on target systems.
#
